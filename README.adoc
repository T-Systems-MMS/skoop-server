...
MySkills Server uses OpenID Connect for authentication via an external identity provider. The application expects an ID token to be passed as bearer token with each API request.

Each request must contain the appropriate `Authorization` header:

    Authorization: Bearer <ID token>

During development a local https://www.keycloak.org/[KeyCloak] server can be used to manage test users and create token. It can be easily set up with Docker:

    docker run --name keycloak -p 9000:8080 -e KEYCLOAK_USER=admin -e KEYCLOAK_PASSWORD=admin jboss/keycloak:4.3.0.Final

The authorization server must have a realm called `MySkills` containing a client registration with the ID `myskills`. There is an export of a suitable test realm in `tools/keycloak/myskills-realm.json` which can be imported into the KeyCloak server.

In order to obtain an ID token you must call the following URL in your browser:
...
After logging in to the authorization server you will be redirected to http://localhost:4200/ with the ID token passed in the `id_token` query parameter. This URL is preconfigured as valid redirect URL in the test realm.

The authorization server must provide the user name in the `user_name` claim of the issued ID tokens. This is preconfigured in the test realm.

If you want to use a different port for the KeyCloak server or different names for realm and client you have to adjust the MySkills configuration accordingly (see `src/main/resources/application.yml`).

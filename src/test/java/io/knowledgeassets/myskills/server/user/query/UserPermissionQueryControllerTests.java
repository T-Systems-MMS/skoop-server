...
class UserPermissionQueryControllerTests extends AbstractControllerTests {
	...
	void testGettingAllUserPermissionsGrantedToOtherUsers() throws Exception {
		...
		mockMvc.perform(get("/users/adac977c-8e0d-4e00-98a8-da7b44aa5dd6/permissions")
				...
				.andExpect(jsonPath("$[0].owner.academicDegree", is(equalTo("Diplom-Wirtschaftsinformatiker"))))
				.andExpect(jsonPath("$[0].owner.positionProfile", is(equalTo("Software Developer"))))
				.andExpect(jsonPath("$[0].owner.summary", is(equalTo("Developer"))))
				.andExpect(jsonPath("$[0].owner.industrySectors", is(equalTo(Arrays.asList("Automotive", "Telecommunication")))))
				.andExpect(jsonPath("$[0].owner.specializations", is(equalTo(Arrays.asList("IT Consulting", "Software Integration")))))
				.andExpect(jsonPath("$[0].owner.certificates", is(equalTo(Collections.singletonList("Java Certified Programmer")))))
				.andExpect(jsonPath("$[0].owner.languages", is(equalTo(Collections.singletonList("Deutsch")))))
				...
				.andExpect(jsonPath("$[0].authorizedUsers[0].academicDegree", is(equalTo("Diplom-Wirtschaftsinformatiker"))))
				.andExpect(jsonPath("$[0].authorizedUsers[0].positionProfile", is(equalTo("Software Architect"))))
				.andExpect(jsonPath("$[0].authorizedUsers[0].summary", is(equalTo("Architect"))))
				.andExpect(jsonPath("$[0].authorizedUsers[0].industrySectors", is(equalTo(Arrays.asList("Automotive", "Telecommunication")))))
				.andExpect(jsonPath("$[0].authorizedUsers[0].specializations", is(equalTo(Arrays.asList("IT Consulting", "Software Integration")))))
				.andExpect(jsonPath("$[0].authorizedUsers[0].certificates", is(equalTo(Collections.singletonList("Scala Certified Programmer")))))
				.andExpect(jsonPath("$[0].authorizedUsers[0].languages", is(equalTo(Collections.singletonList("English")))));
	}

...
class SkoopJwtAuthenticationConverterTests {

	private SkoopJwtAuthenticationConverter skoopJwtAuthenticationConverter;

	@BeforeEach
	void setUp() {
		skoopJwtAuthenticationConverter = new SkoopJwtAuthenticationConverter();
	}
	...
	void rolesAreExtractedAsGrantedAuthoritiesBothFromRealmAndResourceAccess() throws Exception {
		...
		try (final InputStream is = body.getInputStream()) {
			...
			Collection<GrantedAuthority> grantedAuthorities = skoopJwtAuthenticationConverter.extractAuthorities(new Jwt("token",

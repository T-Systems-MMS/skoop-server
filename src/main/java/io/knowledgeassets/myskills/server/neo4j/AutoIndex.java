...
class AutoIndex {
	...
	public static Optional<AutoIndex> parse(String description) {
		...
		if (matcher.matches()) {
			...
			String[] properties = matcher.group("property").split(",");
			return of(new AutoIndex(UNIQUE_CONSTRAINT, label, properties));
		}
		...
		if (matcher.matches()) {
			String label = matcher.group("label").trim();
			String[] properties = matcher.group("property").split(",");
			return of(new AutoIndex(NODE_PROP_EXISTENCE_CONSTRAINT, label, properties));
		}
		...
		logger.warn("Could not parse neo4j description {}", description);
